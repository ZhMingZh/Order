...
def uploadPic():
    ...
    if upfile is None:
        return '<script type="text/javascript">{0}.error("{1}")</script>'.format(callback_target, '上传失败-1')

    ret = UploadService.uploadByFile(upfile)
    if ret['code'] != 200:
        return '<script type="text/javascript">{0}.error("{1}")</script>'.format(callback_target, '上传失败-2')
